...
在 Web 审计看板 (`/audit`) 点击 **🚀 一键上架** 按钮，系统自动：
1. 查询 SteamPy 市场价格
2. 智能定价（比市场最低价低 ¥0.50）
3. 利润校验（亏本或利润不足自动跳过）
4. 使用 AI 匹配的游戏名自动上架
...
```python
# 自动 undercut（可配置）
target_price = market_lowest - ¥0.50

# 利润校验
expected_revenue = target_price × 0.97  # 扣除 3% 手续费
expected_profit = expected_revenue - purchase_cost

# 熔断条件
if expected_profit < ¥0.50 or ROI < 5%:
    跳过（不上架）
```

### 🔒 安全说明

**飞书通知中不显示 CDKey**（保护敏感信息）：
- ✅ 使用订单编号标识：`ORD_HHMMSS_XXX`
- ✅ 显示游戏名称、成本、价格、利润
- ✅ 显示手续费、预期收入、ROI
- ❌ 不显示任何 CDKey 信息
...
# --- 🚀 自动上架配置 ---
AUTO_LISTER_CONFIG = {
    "UNDERCUT_AMOUNT": 0.50,     # 比市场低多少元
    "MIN_PROFIT_MARGIN": 0.50,   # 最低利润要求 (元)
    "MIN_ROI": 0.05,             # 最低 ROI (5%)
}
```
...
- ✅ **逐字键入模式** - 禁用 `fill()`，改用 50ms/字符 逐字键入
- ✅ **随机延迟** - 商品之间 1 秒延迟
- ✅ **频率控制** - 防止请求过快
...
| 文件 | 说明 |
|------|------|
| `auto_lister.py` | 自动上架引擎核心 |
| `web_dashboard.py` | Web API 接口 |
| `audit_dashboard.html` | 前端界面 |
| `SteamPY_Scout/steampy_hunter.py` | 上架执行（逐字键入） |
...
详细快速启动指南请查看：[QUICK_START.md](QUICK_START.md)
